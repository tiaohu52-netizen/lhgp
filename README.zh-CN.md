...
**说人话：它是一个本机调度器。你把长期目标拆成阶段、写清每阶段交什么、怎么算通过；它按 Deadline 把阶段任务自动发给执行者（任意 CLI Agent 或普通程序），把产出交给独立验收者，按结果推进或带着失败证据重发；人只在里程碑上签字。**

它不依赖特定会话或模型：调度内核不需要 LLM（就是 SQLite + 状态机 + 文件），
模型只在干活和验收时被调用。设计上刻意老派——不靠长上下文魔术，靠可查的账本和可复算的检查。
“远期目标”是主要使用场景；“限期合同中枢”只是产品名。

### 和「一份文档 + hook」有什么不同

| 问题 | 文档 + hook | LHGP |
|---|---|---|
| 状态在哪 | 在会话或人脑里，靠自觉维护 | 本机 SQLite：谁在做、做到哪、预算剩多少，可查 |
| 完成怎么算 | 靠人说或模型自己说 | 预置检查（文件存在、正则、命令退出码…）+ 独立验收者的结构化判定 |
| 失败怎么定义 | 没定义 | 检查不过或验收不过即失败，**失败证据随 attempt 落账** |
| 修复谁做 | 人再发一遍 | 调度器带着失败证据自动重派（换执行者或原执行者）；预算耗尽才升级给人 |
| 崩溃/换会话 | 断片，要靠人接 | 每次尝试都从上下文快照 + 交接文档重建，账本不丢 |
| 留痕 | hook 日志 | 决策 / attempt / 验收三段历史，可逐条追溯 |

### 最短路径：三步跑一个长目标（尽量少人工）

1. **一次写好计划**：`goal/update` 给全计划——阶段数组 + 每阶段的机器可判检查
   （如 `{"judge":"machine","kind":"file-exists","target":"out.md"}`）+
   `plan.pre_authorized`（用户预授权：动作白名单 + 预算增量上限）。
2. **开着 daemon**：`lhgp start`（建议配看门狗）。某阶段完成后，调度器会
   **自动合成下一阶段合同 → 按预授权自动批准 → 派工**；不需要逐阶段手工 prepare/approve。
3. **只在最后签一次字**：中间阶段用 `judge:machine`（机器闸自判）；最后一道留
   `{"judge":"user"}`，通过后 `lhgp contract user-confirm <合同 id>`。

> 判据越具体，越不需要人工看会话：把「做完什么」写成 1–3 条可复算的检查即可。
> 想看进展用 `lhgp board` / `lhgp brief`，不必翻聊天记录。
> 预授权样例（`plan.pre_authorized`）：`{"enabled":true,"wildcard":true,"actions":["read file","run command","write file","search code","verify acceptance","ask user"],"max_budget_increment":5,"max_spec_changes":5}`。
