...
def set_attempt_state(
    ...
    *,
    ...
    """更新 attempt 状态（§7.4 轴）。终态写入 terminal_at，非终态保留原值。

    不在此处校验迁移合法性：状态机是纯函数（state_machine.py），
    调用方负责先判定；本模块只做忠实落库。

    非终态写入**不清空** ``terminal_at``：旧实现写的是 ``?``（非终态传
    ``None``），于是一条已经终态的 attempt 若被任何路径再写一次非终态
    状态，终态时间就被抹成 NULL——「终态不可变」（SPEC §7.4）在存储层
    没有兜底。改为 CASE，只在本次是终态时写入。
    """
