...
class TestPortfolioSnapshotSerialization:
    """``PortfolioSnapshot.to_dict`` 必须能序列化**非空**组合。

    回归：``to_dict`` 曾用 ``c.__dict__`` 读 ``ContractSummary``，而后者是
    ``slots=True`` 冻结 dataclass——实例没有 ``__dict__``。于是空组合侥幸
    通过（列表推导不执行），任何非空组合直接 ``AttributeError``，也就是
    ``lhgp portfolio`` 与 MCP ``tool_portfolio`` 对真实数据必崩。
    """

    def test_empty_snapshot_serializes(self, tmp_path: Path) -> None:
        conn = _setup(tmp_path)
        try:
            payload = portfolio_summary(conn).to_dict()
        finally:
            conn.close()
        assert payload["count"] == 0
        assert payload["contracts"] == []

    def test_non_empty_snapshot_serializes(self, tmp_path: Path) -> None:
        conn = _setup(tmp_path)
        try:
            _save(conn, "lt-pf-1")
            payload = portfolio_summary(conn).to_dict()
        ...
        assert row["contract_id"] == "lt-pf-1"
        # 全字段都在（防 asdict 之外的手工裁剪漏字段）
        assert {
            "contract_id",
            "title",
            "state",
