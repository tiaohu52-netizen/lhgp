...
def handle_attempt_write_back(
    ...
    **kwargs: Any,
    ...
    # Per-attempt session token（安全加固）：证明「写回方就是这个 attempt
    # 被拉起的那个进程」，而不是另一个恰好也持有主令牌的客户端。
    #
    # 旧判定是 ``client_id == "executor"``：MCP 客户端（模型）与 CLI 都被
    # 豁免，于是任何能连上本机 RPC 的调用方都能替某个 attempt 写回——
    # verifier 是写验收证据的一方，伪造它的写回等于直接伪造验收结论。
    #
    # 改为：除 daemon（内部调用，走主令牌已可信，且必须能结算终态 attempt）
    # 外，只要该 attempt 签发过凭据就必须出示。
    if envelope.client_id != "daemon":
        import hashlib
        ...
        if stored_hash is not None:
            ...
            if hashlib.sha256(session_token.encode()).hexdigest() != stored_hash:
                raise RpcError(
                    code=ErrorCode.VALIDATION_FAILED,
                    message="session_token mismatch; "
                    "use the LHGP_SESSION_TOKEN environment variable from spawn",
                )
        # 从未签发凭据的 attempt（历史数据 / 非 spawn 路径创建）保持旧行为：
        # 不凭一次改动让存量数据突然失联。新签发的 attempt 一律强制。
