...
**Single exception to "never overflow".** When the *first* (highest-score)
memory alone does not fit, it is truncated rather than dropped, so the
section still surfaces one row with a greppable ``[…truncated…]`` marker.
That truncated row can still exceed a budget smaller than
``_HEADER_BYTES + _ITEM_OVERHEAD_BYTES`` — this is deliberate (an empty
memory section is worse than a clipped one) and pinned by
``tests/unit/test_memory_index.py::TestCapacityContract``. Every other
path respects the budget exactly, including the 2-byte ``\n\n`` separator
...
class MemoryIndex:
    ...
    def _fit_to_budget(self, items: list[RetrievedMemory]) -> list[RetrievedMemory]:
        ...
        Accounting must mirror ``_render_section`` exactly, which emits
        ``HEADER + "\\n\\n".join(items) + "\\n"``: a 1-byte trailing
        newline plus a **2-byte** separator between adjacent items. The
        old accounting charged ``+1`` per item, undercounting by ``n-1``
        bytes for ``n`` items — so a section that "fit" the budget could
        still overflow it (3 items overflowed by 2 bytes), breaking the
        module's capacity contract and the downstream snapshot limit.
        """
        selected: list[RetrievedMemory] = []
        total = _HEADER_BYTES + 1  # trailing newline emitted by _render_section
        for item in items:
