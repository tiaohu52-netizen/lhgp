...
def test_synthesize_fallback_marker_is_observable_not_artifact_present(tmp_path: Path) -> None:
    """阶段 spec 无机器 check 时的占位标记必须是 observable 且非
    mandatory——历史实现用 ``artifact-present: stage:<id>``，评估器把它
    当文件路径、恒判 fail（协议 fail 优先于模型判定），自动合成的阶段
    合同在验收处必失败。"""
    goal: dict = {"objective": "x"}
    stage: dict = {
        "id": "s1",
        "title": "实现",
        "spec": {"all": [{"judge": "user", "question": "done?"}]},
    }
    draft = _synthesize_stage_draft(goal, stage, previous_evidence=None, now=NOW)
    checks = draft["acceptance"]["checks"]
    assert len(checks) == 1
    assert checks[0]["kind"] == "observable"
    assert checks[0]["target"] == "stage:s1"
