...
def _replayed_events(
    ...
    """幂等键（request_id）的归属校验，返回可安全早退的历史事件。

    ``events`` 表按 ``request_id`` 检索时**不带 contract_id 过滤**
    （见 ``lhgp/persistence/events_query.get_events_by_request_id``），
    所以「这个 request_id 见过」并不等于「这个 request_id 属于本合同」。
    之前三处写入口只判断「非空即早退」，于是跨合同复用同一 request_id
    会被静默当成成功——调用方以为写成了，真实状态变更被丢弃；``write_back``
    还会把他合同的 event_id 当成本次结果返回（信息泄漏）。

    fail-closed：request_id 命中但他合同的事件时抛
    ``IdempotencyMismatchError``（RPC 边界已把它映射为
    ``IDEMPOTENCY_REPLAY_MISMATCH``），而不是猜一个结果。``contract_id``
    为空的旧事件视为不归属。
    """
    if not request_id:
        return []
    existing = get_events_by_request_id(conn, request_id)
    if not existing:
        return []
    owners = {e.contract_id for e in existing}
    if owners != {contract_id}:
        foreign = sorted(str(o) for o in owners if o != contract_id)
        raise IdempotencyMismatchError(
            f"request_id {request_id!r} already belongs to another contract "
            f"({', '.join(foreign) or 'unattributed events'}); "
            f"idempotency keys must not be shared across contracts"
        )
    return existing
...
def update_contract_state(
    ...
    *,
    ...
    """
    with transaction(conn):
        existing_events = _replayed_events(conn, request_id, contract_id)
        if existing_events:
