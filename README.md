...
**Plain version: a local scheduler.** You split a long goal into stages and write down what each stage
must deliver and how it is checked; LHGP sends stage tasks to executors (any CLI agent or plain program),
hands results to an independent verifier, advances or re-dispatches with failure evidence — and you only
sign off at milestones. It is model- and session-independent, and the scheduling core itself needs no LLM
(SQLite + a state machine + files — deliberately old-fashioned, not a long-context trick).
...
## How this differs from a doc + hook

| Question | Doc + hook | LHGP |
|---|---|---|
| Where does state live? | In the session/heads, by convention | Local SQLite: who is doing what, how far along, budget left — queryable |
| What counts as done? | A human or the model says so | Pre-declared checks (file exists, regex, command exit code…) + independent verifier evidence |
| What counts as failure? | Undefined | A failed check or failed acceptance; **failure evidence is recorded per attempt** |
| Who repairs it? | A human re-sends the task | The scheduler re-dispatches with the failure evidence (same or different executor); escalation only when budget runs out |
| Crash / new session | Loses the thread | Each attempt rebuilds context from `active.md` + handover; the ledger survives |
| Audit | Hook logs | Decision / attempt / acceptance histories, item by item |

## The short path (three steps, minimal manual work)

1. Write the plan once (`goal/update` or MCP `submit_plan`): stages + machine-checkable checks per stage
   (e.g. `{"judge":"machine","kind":"file-exists","target":"out.md"}`) + `plan.pre_authorized`
   (user pre-authorisation: action allow-list + budget increment caps).
2. Keep the daemon running (`lhgp start`; a watchdog is recommended). When a stage completes, the
   scheduler **auto-synthesizes the next stage's contract → auto-approves it under the pre-authorisation
   → dispatches**; no per-stage prepare/approve by hand.
3. Sign once at the end: intermediate stages use `judge:machine`; leave the final stage as
   `{"judge":"user"}` and confirm with `lhgp contract user-confirm <id>`.

> The more concrete the checks, the less anyone needs to read session transcripts.
> Use `lhgp board` / `lhgp brief` for progress instead of scrolling chat logs.
> `plan.pre_authorized` example: `{"enabled":true,"wildcard":true,"actions":["read file","run command","write file","search code","verify acceptance","ask user"],"max_budget_increment":5,"max_spec_changes":5}`.
