...
from lhgp.templates import available, load
...
class TestShippedTemplates:
    """随包模板必须自己能过自家校验（读真模板，而不是合成草稿）。

    回归（两个都随包发布、此前无人守）：
      * ``research.json`` 里 ``"\\[source\\]"`` 是非法 JSON 转义（``\\[``）——
        整个文件 ``json.loads`` 直接抛 ``JSONDecodeError``，模板完全不可用；
      * ``release-check.json`` 的 ``max_escalations: 0`` 被本模块与
        ``contracts/budget`` 同时判非法——模板过不了自家校验。

    既有测试只用 ``_write(tmp_path, ...)`` 合成草稿，从不读 ``src/lhgp/
    templates/*.json``，所以两个缺陷都能长期存活。
    """
    ...
    def test_template_is_valid_json(self, name: str) -> None:
        ...
        for field in ("title", "objective", "deadline_at", "acceptance", "budget"):
            assert field in payload, f"{name}.json 缺字段 {field}"

    @pytest.mark.parametrize("name", available())
    def test_template_has_no_structural_budget_problems(self, name: str, tmp_path: Path) -> None:
        path = tmp_path / f"{name}.json"
        path.write_text(load(name), encoding="utf-8")
        problems = validate_draft_file(path)
        # 模板是给人填空的骨架：占位符（<...>）与尚未创建的交付物会照常报警，
        # 这是设计意图。但 budget.* 是结构属性，与用户填什么无关——模板自己
        # 必须先合法，否则用户一改完 deadline 就被拒。
        assert [p for p in problems if p.startswith("budget.")] == []
