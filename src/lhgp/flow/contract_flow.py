...
# Windows 盘符（``C:foo`` / ``C:\foo``）：盘符相对路径在 Path 语义下既非
# 绝对路径也不含 ``..``，必须按字面拦（与 claims_check 同一类陷阱）。
...
def _is_within(root: Path, candidate: Path) -> bool:
    """``candidate`` 是否仍在 ``root`` 之内（解析符号链接/``..`` 之后判定）。"""
    try:
...
def _resolve_by_module_path(src_root: Path, seed: str) -> Path | None:
    """Resolve a dotted module seed to a file **inside** ``src_root``.

    Seed values come from contract acceptance checks, i.e. from user/模型
    提供的数据，所以必须当作不可信。``src_root / seed`` 对这些写法不设防：
      * ``../../etc/passwd``  → 逃出源码根；
      * ``/abs/path``         → Windows 上 ``Path.__truediv__`` 不视为绝对；
      * ``C:foo``             → Windows 盘符相对路径，会**替换盘符**。
    若不在校验前拦下，``walk_contract`` 会先 ``read_text()`` 把文件读进
    内存，随后才被 ``_module_path_from_file`` 的 ``relative_to`` 拒掉
    —— 拒绝来得太晚，读取已经发生。
    """
    if seed.startswith(("/", "\\")) or _DRIVE_RE.match(seed) or ".." in Path(seed).parts:
        logger.warning("contract_flow: refusing out-of-tree seed %r", seed)
        return None
    ...
    for candidate in candidates:
        if _is_within(src_root, candidate) and candidate.is_file():
            return candidate
    ...
    *,
    ...
    for seed in seeds:
        for file_path in _resolve_seed(root, seed):
            ...
            try:
                # 先做包含性校验再读：``_module_path_from_file`` 内部走
                # ``relative_to(src_root)``，是这里唯一的源码根约束。放在
                # ``read_text`` 之后就形同虚设——越界文件已经被读进内存了。
