...
class TestActiveContract:
    def test_writes_one_file_with_expected_frontmatter(self, tmp_path: Path) -> None:
        ...
        # id/title 现在加引号（见 TestFrontmatterIsValidYaml）：
        # 索引器用 value.strip("\"'") 解析，两种写法都能读。
        assert 'id: "lt-active-1"\n' in page
        assert 'title: "Sample contract"\n' in page
...
class TestFrontmatterIsValidYaml:
    """frontmatter 的 scalar 必须加引号——契约标题是用户自由文本。

    回归：``id:``/``title:`` 直接插值未加引号。标题里出现 ``:``（"Phase 1: 收尾"
    这种太常见）或 ``#`` 时，**真正的 YAML 解析器**（Obsidian）会读错或读不到，
    而且不报错。``_yaml_list`` 也只转义了双引号、没转义反斜杠。
    """

    @staticmethod
    def _render(title: str) -> str:
        from lhgp.wiki.sync import _render_page

        view = SimpleNamespace(
            contract_id="lt-fm-1",
            state=SimpleNamespace(value="active"),
            acceptance_status=SimpleNamespace(value="pending"),
            deadline_status=SimpleNamespace(value="not_due"),
            draft=SimpleNamespace(
                title=title,
                objective="o",
                deadline_at=NOW + timedelta(hours=2),
                context={"tags": []},
                acceptance=SimpleNamespace(standard="s", checks=("c",)),
            ),
        )
        return _render_page(
            view,
    ...
    def test_colon_in_title_is_quoted(self) -> None:
        page = self._render("Phase 1: 收尾")
        assert 'title: "Phase 1: 收尾"' in page

    def test_hash_in_title_is_quoted(self) -> None:
        page = self._render("sprint #3")
        assert 'title: "sprint #3"' in page

    def test_double_quote_is_escaped(self) -> None:
        page = self._render('say "hi"')
        assert "title: " in page and '\\"hi\\"' in page

    def test_backslash_is_escaped(self) -> None:
        """反斜杠必须先于引号转义，否则 ``a\\`` 会吃掉后面的闭合引号。"""
        # 用 chr(92) 拼，避免源码里多层转义把字面量本身搞坏
        bs = chr(92)
        page = self._render(f"path C:{bs}{bs}tmp")
        assert f'title: "path C:{bs * 4}tmp"' in page

    def test_newline_is_escaped(self) -> None:
        page = self._render("line1\nline2")
        assert "title: " in page and "\\n" in page

    def test_tags_escape_backslash(self) -> None:
        from lhgp.wiki.sync import _yaml_list
