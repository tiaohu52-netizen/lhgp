...
class TestMCPErrors:
    ...
    def test_unknown_notification_status_returns_invalid_params(self, data_dir: Path) -> None:
        ...
        try:
            ...
            assert resp["error"]["code"] == -32602
            # schema 里早就声明了 ``"status": {"enum": [...]}}``，但
            # ``_validate_arguments`` 此前只查 required/类型/未知键，enum
            # 形同注释——非法值一路走到 notifications.py 才被拒。现在在
            # 协议边界就拦下，并把合法值一并告诉调用方（比下游的单句更可行动）。
            message = resp["error"]["message"]
            assert "status" in message
            assert "pending" in message and "leased" in message and "sent" in message
        finally:
            _stop_mcp(proc)
...
class TestSchemaConstraintsAreEnforced:
    """inputSchema 里声明的约束必须在服务端被强制。

    回归：``_validate_arguments`` 只查 required / 类型 / 未知键，其余关键字
    一律放过。于是 schema 写了 ``enum``、``minimum``、``maximum`` 也只是注释
    ——MCP host 不会替服务端校验，约束形同虚设。
    """
    ...
    @staticmethod
    def _err(args: dict) -> str:
        from longtask.mcp_server import _validate_arguments

        msg = _validate_arguments(args, TestSchemaConstraintsAreEnforced.SCHEMA)
        assert msg is not None, f"expected a validation error for {args}"
        return msg

    @staticmethod
    def _ok(args: dict) -> None:
        from longtask.mcp_server import _validate_arguments

        assert _validate_arguments(args, TestSchemaConstraintsAreEnforced.SCHEMA) is None

    def test_enum_is_enforced(self) -> None:
        assert "must be one of" in self._err({"status": "queued"})

    def test_enum_valid_value_passes(self) -> None:
        self._ok({"status": "pending"})

    def test_minimum_and_maximum_are_enforced(self) -> None:
        assert "must be >=" in self._err({"limit": 0})
        assert "must be <=" in self._err({"limit": 201})

    def test_numeric_bounds_accept_boundaries(self) -> None:
        self._ok({"limit": 1})
        self._ok({"limit": 200})

    def test_string_length_bounds_are_enforced(self) -> None:
        assert "at least 2" in self._err({"name": "x"})
        assert "at most 8" in self._err({"name": "x" * 9})

    def test_array_item_bounds_are_enforced(self) -> None:
        assert "at least 1" in self._err({"tags": []})
        assert "at most 3" in self._err({"tags": [1, 2, 3, 4]})

    def test_pattern_is_enforced(self) -> None:
        assert "must match pattern" in self._err({"slug": "ABC-1"})

    def test_pattern_valid_value_passes(self) -> None:
        self._ok({"slug": "abc"})

    def test_absent_optional_keys_still_pass(self) -> None:
        """没传的可选参数不得因为约束而被拒。"""
        self._ok({})
