...
def compile_context_snapshot(
    ...
    *,
    ...
    # 装 1-2 条小记忆；但**上限必须收敛到合同声明的总容量**——
    # max_bytes 可小至 1（见 _ContextPolicy 解析），旧写法只有下限没有
    # 上限，于是 max_bytes < 3750 的合同光 memory 段就超限，快照恒定
    # 抛 CapacityRefusedError，attempt 永远起不来（fail-closed 变成
    # fail-permanent）。
    memory_budget = min(int(policy.max_bytes * 0.4), 4000)
    memory_budget = min(max(memory_budget, 1500), policy.max_bytes)
