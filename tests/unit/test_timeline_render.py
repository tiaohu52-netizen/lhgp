...
class TestActorFieldIsNotAnInjectionVector:
    """actor / attempt_id 同样进 DOM，也必须转义。

    回归：只有 ``summary`` 被 ``html.escape``，``actor`` 与 ``attempt_id``
    原样进入 ``innerHTML``；且整份 ``const DATA = {...}`` 直接内联进
    ``<script>``，字段里出现 ``</script>`` 就能提前闭合宿主标签。
    actor 可由 RPC 客户端决定（``resolve_actor`` 允许注册客户端），
    所以这是可达的注入面，不只是理论问题。
    """

    def test_actor_html_is_escaped_in_row(self, conn: sqlite3.Connection) -> None:
        _seed_contract(conn, "c-actor")
        append_event(
            conn,
            contract_id="c-actor",
            event_type=EventType.ATTEMPT_FAILED,
        ...
        page, _ = build_timeline_html(conn, contract_id="c-actor", now=NOW)
        row = _data_rows(page)[-1]
        assert "<img src=x" not in str(row["actor"])
        assert html.escape("<img src=x onerror=alert(1)>") == row["actor"]

    def test_script_close_tag_cannot_break_out(self, conn: sqlite3.Connection) -> None:
        _seed_contract(conn, "c-breakout")
        append_event(
            conn,
            contract_id="c-breakout",
            event_type=EventType.ATTEMPT_FAILED,
            payload={"reason": "x</script><script>alert(1)</script>"},
            now=NOW + timedelta(seconds=1),
        )
        page, _ = build_timeline_html(conn, contract_id="c-breakout", now=NOW)
        # 宿主 script 块内不得出现未转义的闭合标签
        assert "</script><script>alert(1)" not in page
        # 数据仍可被正常解析回来（转义不改变语义）
        row = _data_rows(page)[-1]
        assert "alert(1)" in str(row["summary"])
