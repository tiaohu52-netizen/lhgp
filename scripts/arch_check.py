...
def _package_parts(relative: Path) -> list[str]:
    """The importable package parts of a source file, e.g.

    ``longtask/cli/tick.py`` -> ``["longtask", "cli"]``.
    """
    return list(relative.parts[:-1])


def imported_modules(tree: ast.AST, package_parts: list[str]) -> list[tuple[str, int]]:
    """Extract module-level import targets as **absolute** module paths.

    Three shapes must all be resolved, because each one was a blind spot
    that let a forbidden layer hop through the gate:

    1. ``import a.b``            -> ``a.b``
    2. ``from a import b``       -> ``a`` **and** ``a.b`` (``b`` may be a
       submodule; the old code recorded only ``a``, so
       ``from longtask.adapters import subprocess_adapter`` never matched
       the ``adapters.subprocess_adapter`` rule);
    3. ``from ..x import y``     -> resolved against the file's own
       package (the old code dropped ``node.level != 0`` entirely, so every
       relative import was invisible).

    Over-reporting is safe here: rule matching is exact-or-dotted-prefix,
    so ``from a.b import ClassName`` adds a candidate that simply never
    matches a layer rule.
    """
...
def rule_violated(layer: str, module: str) -> str | None:
    """Return the violated rule string for ``layer`` importing ``module``.

    Pure and separately testable: the gate's value depends entirely on this
    predicate firing, so it must be exercised by a positive control rather
    than only by "the repo currently has zero violations".
    """
...
def check_file(path: Path) -> list[Violation]:
    ...
    for module, lineno in imported_modules(tree, _package_parts(relative)):
        if rule_violated(layer, module) is None:
            continue
        violations.append(
            Violation(
