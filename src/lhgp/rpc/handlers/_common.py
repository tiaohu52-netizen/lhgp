...
from lhgp.contracts.validation import CONTRACT_ID_PATTERN, is_safe_contract_id
...
# 防线是「无路径字符」而非命名风格。
#
# 正则与判定逻辑已上移到 ``lhgp/contracts/validation.py``（单一事实来源），
# CLI 侧直连 DB 的命令（如 ``feedback diff``）复用同一实现，避免两处漂移。
_CONTRACT_ID_RE = CONTRACT_ID_PATTERN


def _is_safe_contract_id(contract_id: str) -> bool:
    return is_safe_contract_id(contract_id)
