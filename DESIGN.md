...
> 文档修订：v0.8（不可信外部文本的入站边界：输出清洗 + 可验证注入围栏；进程树终止）
> 协议版本：v0.1（草案；线协议与 schema 版本独立于本文档修订号）
> 日期：2026-09-10
> 状态：设计阶段；本版把 §14.1 里两条原本只是「宣称」的威胁处置——提示词注入污染交接、注入 shell 命令——升级为可验证的结构（围栏 token 由正文摘要派生，正文无法提前闭合），并补上取消时的进程树终止。线协议、schema、错误码均不变。
