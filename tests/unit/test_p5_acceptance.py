...
class TestEvaluateCheck:
    ...
    def test_artifact_present_stage_marker_defers_to_verifier(self, tmp_path) -> None:
        """阶段占位标记 ``stage:<id>`` 不是 workspace 工件：协议层必须
        如实记 undetermined 交给 §12.4 verifier 判定块，而不是当路径查、
        伪造 fail——协议 fail 优先于模型判定，会让自动合成的阶段合同在
        验收处一票否决（stage_synth 占位修复的协议侧回归）。"""
        marker = evaluate_check(
            CheckSpec(kind=CheckKind.ARTIFACT_PRESENT, target="stage:M2"),
            workspace_root=tmp_path,
        )
        assert marker.outcome == "undetermined"
        assert "stage-completion marker" in marker.details
        # 真实路径目标保持确定性行为：缺文件 → fail。
        missing = evaluate_check(
            CheckSpec(kind=CheckKind.ARTIFACT_PRESENT, target="missing.bin"),
            workspace_root=tmp_path,
        )
