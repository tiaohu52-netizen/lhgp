...
def is_repo_relative(path: str) -> bool:
    """Is ``path`` a plain, repo-relative path?

    Windows drive-relative forms (``C:foo``) must be rejected explicitly:
    ``Path("C:foo").is_absolute()`` is ``False`` and its ``parts`` contain
    no ``..``, so the old check passed them — while ``REPO_ROOT / "C:foo"``
    on Windows **replaces the drive**, making the existence probe point
    ...
    if _DRIVE_RE.match(path):
        return False
    # POSIX 绝对路径在 Windows 上 ``is_absolute()`` 为 False（没有盘符就
    # 不算绝对），所以必须按字面拦前导分隔符——否则同一份 registry 在
    # Windows 门里通过、在 Linux 门里才红，等于平台相关地漏检。
    if path.startswith(("/", "\\")):
        return False
    p = Path(path)
...
def main() -> int:
    ...
    errors: list[str] = []
    # format_checker 必须显式传入：Draft202012Validator 默认**不**执行
    # ``"format": "date"`` 之类的断言，于是 schema 里声明的时间字段
    # 任何字符串都能通过——声明的时间戳形同虚设。
