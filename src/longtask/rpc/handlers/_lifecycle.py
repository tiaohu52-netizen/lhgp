...
Failure is silent **to the caller** — the next ``goal/next`` call will
surface ``create_contract`` so the caller can re-attempt with full
authority. That is a deliberate protocol choice, but it must not mean
"invisible": an automatic stage-contract creation that dies in here with
no trace is undiagnosable, so both failure paths log at WARNING.
"""
...
def auto_create_next_stage_contract(
    ...
    envelope = RequestEnvelope(
        ...
        client_id="daemon",
        # 硬编码 2 与实际线协议版本（``PROTOCOL_VERSION``）脱钩：协议升版时
        # 这里会静默滞后，而版本不符在服务端是 fail-closed 拒收。
    ...
    try:
        ...
        # 对调用方静默（见模块 docstring），但必须留痕：否则「阶段合同没被
        # 自动创建」这类问题在日志里完全不可见，只能靠猜。
        logger.warning(
            "auto stage-contract prepare failed (stage=%s goal=%s cid=%s): %s",
            next_stage_id,
            contract.goal_id,
            new_cid,
            exc,
    ...
    try:
        ...
        logger.warning(
            "auto stage-contract approval failed (cid=%s): %s",
            new_cid,
            exc,
            exc_info=True,
        )
