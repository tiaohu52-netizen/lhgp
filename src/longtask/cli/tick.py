...
def run_daemon_tick(
    ...
    #
    # 「已接线」此前是假的：这里只调了 ``record_dispatch``，从不调
    # ``can_dispatch``，于是 ``max_dispatches_per_tick`` 无论配成什么都无效
    # （注释与 fairness.py 的 docstring 都声称规则 2 已生效）。现在在派工前
    # 真的问一次；默认 cap=0（不限）所以行为不变，配了才有约束力。
    capacity_ledger = TickCapacityLedger(fairness_config or FairnessConfig())
    ...
    for c in ordered_contracts:
        ...
        match decision.tier:
            ...
            case UrgencyTier.RESPAWN | UrgencyTier.PARALLEL:
                # 派工时间窗（execution.dispatch_window）：窗口外不派工。
                # 常规路径下回头点已被钉在窗口起点（见 _compute_next_decision_at）；
                # 这一层是竞态/无回头点旧合同的兜底，并留一条 deferred 审计。
                window = _dispatch_window(c)
                if window is not None and not lease_alive:
                    inside, next_start = _window_state(now, window)
                    if not inside and next_start is not None:
                        _defer_for_window(
                            conn, c, now=now, window=window, next_start=next_start, emit=_emit
                        )
                        continue
                ...
                # per-tick 容量帽：同 tick 已派过、或本 tick 派工数已达
                # max_dispatches_per_tick → 本轮跳过，下一 tick 再争。
                # 默认 cap=0（不限），所以不配置时行为与既往完全一致。
                if not capacity_ledger.can_dispatch(cid):
                    _emit(
                        f"promoter/deferred-tick-capacity:{cid}:"
                        f"dispatched={len(dispatched_this_tick)}"
                    )
                    continue
...
def _remaining_workload_hours(root: Path, contract: Any) -> float:
    ...
    if data is None or violations or not data.source_attempt_id.strip():
        return initial
    # provisioning 阶段写入的合成占位（source=init）不是任何真实 attempt 的
    # 交接：其 estimate 只是准备时刻的初始快照。若把它当真，用户
    # ``contract patch --workload-hours`` 的修订会被静默压回（2026-09-14
    # 实测：patch 48h 后 urgency 仍按 24h 计算，合同停在派工阈值下不动）。
    if data.source_attempt_id.strip().lower() == "init":
        return initial
...
def _compute_next_decision_at(
    ...
    *,
    ...
    # 派工时间窗（execution.dispatch_window，用户 2026-09-15 令·夜间羊毛模式）：
    # 窗口外且无活租约时，回头点不早于窗口起点——白天不派工、也不空转复核。
    window = _dispatch_window(contract)
    if window is not None and not _lease_alive(lease, now):
        inside, next_start = _window_state(now, window)
        if not inside and next_start is not None:
            result = max(result, next_start)
    return result


# ── 派工时间窗（execution.dispatch_window）───────────────────────────────
# 用户 2026-09-15 令（ZCode 夜间羊毛模式）：合同可声明只在本机时间窗口内
# 派工。窗口外：不改派工计划、不烧 dispatch 预算，只把回头点钉到窗口起点
# （见 _compute_next_decision_at 的窗口分支）；若真有窗口外的派工尝试到达
# 派工分支（竞态/无回头点的旧合同），在那里落一条 dispatch/deferred 并跳过。
#
# 语法：draft.execution = {"dispatch_window": {"start": "23:00", "end": "08:00"}}
# 语义：本地墙钟；支持跨午夜（start > end = 当晚 start 到次日 end）；
#       start == end 视为未配置；非法值视为未配置（配置错误不该击穿 tick）。
...
def _dispatch_window(contract: Any) -> tuple[dt_time, dt_time] | None:
    execution = getattr(contract.draft, "execution", None)
    if not isinstance(execution, dict):
        return None
    raw = execution.get("dispatch_window")
...
def _lease_alive(lease: Any, now: datetime) -> bool:
    if lease is None:
        return False
    expiry: datetime = lease.heartbeat_at + lease.timeout
    return bool(expiry > now)


def _window_state(now: datetime, window: tuple[dt_time, dt_time]) -> tuple[bool, datetime | None]:
    """(窗口内?, 下一窗口起点)；窗口内时第二项为 None。窗口用本机墙钟。"""
    start, end = window
    local = now.astimezone()
    t = local.time()
    # 跨午夜窗口（start > end）取「t >= start 或 t < end」，否则取普通区间判断
    inside = start <= t < end if start < end else t >= start or t < end
    if inside:
        return True, None
    next_start = local.replace(hour=start.hour, minute=start.minute, second=0, microsecond=0)
    if t >= start:
        next_start = next_start + timedelta(days=1)
    return False, next_start.astimezone(now.tzinfo)


def _defer_for_window(
    conn: sqlite3.Connection,
    contract: Any,
    *,
    now: datetime,
    window: tuple[dt_time, dt_time],
    next_start: datetime,
    emit: Callable[[str], None],
) -> None:
    """窗口外派工到达时：同一窗口只落一条 deferred 事件，然后跳过本轮。"""
    ...
    for event in reversed(get_events(conn, contract_id=cid)):
        if event.event_type == EventType.DISPATCH_DEFERRED:
            try:
                ...
                payload = {}
            already = payload.get("window_start") == next_start.isoformat()
            break
    if not already:
        append_event(
            conn,
            ...
            payload={
                ...
                "dispatch_window": f"{window[0].strftime('%H:%M')}-{window[1].strftime('%H:%M')}",
                "window_start": next_start.isoformat(),
                "note": "no dispatch until window opens (execution.dispatch_window)",
            },
            now=now,
            actor="daemon",
        ...
        emit(f"promoter/deferred-window:{cid}:until:{next_start.isoformat()}")
...
def _write_repair_brief(
    ...
    prev_stage = "repair"
    # 修复轮的剩余工时沿用当前估算，不再逐轮减半（2026-09-14 实测）：每
    # 次修复都把估算对半砍，会把 urgency 压到派工阈值以下（u<0.5 只剩
    # REMIND，无重派动作），修复轮永远等不到下一次派工——合同卡死在
    # acceptance=failed。真实进展由下一轮 attempt 自己的 handover 汇报，
    # 估算不由失败动作虚构。
    estimate = contract.draft.workload_initial_hours
    completed: tuple[str, ...] = ()
    if handover_path.is_file():
        ...
        if data is not None:
            ...
            estimate = max(0.25, data.estimate_remaining_hours)
