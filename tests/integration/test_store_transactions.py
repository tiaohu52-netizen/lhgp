...
class TestIdempotency:
    ...
    def test_request_id_reuse_across_contracts_is_refused(self, tmp_path: Path) -> None:
        """同一 request_id 不得跨合同复用（幂等键必须绑定合同）。

        回归：``events`` 表按 request_id 检索时**不带 contract_id 过滤**，
        而三处写入口（update_contract_state / patch_contract / write_back）
        只判断「见过就早退」。于是合同 B 复用合同 A 的 request_id 时，
        写入口静默返回 B 的当前状态、真实状态变更被丢弃——调用方以为
        写成功了。``write_back`` 还会把 A 的 event_id 当成 B 的结果返回。
        """
        config = setup_store(tmp_path)
        conn = connect(config)
        try:
            contract_a = "lt-20260831-031"
            contract_b = "lt-20260831-032"
            save_contract(conn, make_draft("合同 A"), contract_id=contract_a, now=NOW)
            save_contract(conn, make_draft("合同 B"), contract_id=contract_b, now=NOW)

            shared_req = "req-shared-across-contracts"
            v1 = update_contract_state(
                conn,
            ...
            # 合同 B 复用同一 request_id：必须 fail-closed，且不改 B 的状态
            with pytest.raises(IdempotencyMismatchError):
                update_contract_state(
                    conn,
            ...
            assert get_contract(conn, contract_b).state == ContractState.DRAFTED
            # 只有 save_contract 自己的 contract/prepared，被拒的写入没留下任何事件
            b_events = get_events(conn, contract_id=contract_b)
            assert [e.event_type for e in b_events] == [EventType.CONTRACT_PREPARED]

            # 同合同的合法重放仍然幂等（不能因为加了归属校验而破坏正常重放）
            v2 = update_contract_state(
                conn,
                contract_id=contract_a,
                new_state=ContractState.ACTIVE,
