...
def _issue_session_token(conn: sqlite3.Connection, attempt_id: str) -> str:
    """为 attempt 签发一次性会话凭据，返回明文（只经 env/输入传给被拉起的进程）。

    凭据的作用是让写回方能证明「我是本次 attempt 被拉起的那个进程」，
    而不是另一个恰好也拿着主令牌的客户端。库里只存 sha256 摘要。

    **两种角色都必须签发**：此前只有 executor 路径签发，verifier 的
    ``session_token_hash`` 恒为 NULL。而 verifier 恰恰是写验收证据的一方——
    缺了它，「要求 verifier 写回出示凭据」就无从谈起，模型客户端可以
    直接调 write-back 伪造验收证据。
    """
    import hashlib
    import secrets
...
class AttemptRunner:
    ...
    def start_attempt(
        ...
        *,
        ...
        try:
            ...
            input_ = dataclasses.replace(
                input_, session_token=_issue_session_token(self._conn, attempt_id)
            )
    ...
    def _dispatch_verifier(self, now: datetime, *, contract_id: str, executor_id: str) -> bool:
        ...
        # 顺序：CAS 占租约 → 编译输入 → prepare/spawn。
        #
        # 旧实现先 spawn 再 acquire_lease，三处后果：
        #   1) acquire 抛 LeaseCASError 时异常击穿 daemon 主循环，而已经拉起的
        #      子进程无人跟踪——孤儿进程继续改工作区，且没有 attempts 行可回收；
        #   2) _build_verifier_input 在 acquire 之前读租约，verifier 拿到的
        #      lease_generation 比实际少 1，写回时 fencing 判定基于错误的代次；
        #   3) 与执行者路径（dispatch.py 先占租约、runner 再 spawn）不一致，
        #      同一份代码里两种顺序，读代码的人无法判断哪个是契约。
        active_lease = get_lease(self._conn, contract_id)
        ...
        except LeaseCASError as exc:
            # 租约被别的持有者抢先：如实失败并归还控制权，不 spawn。
            self._fail_attempt(
                now,
                contract_id,
                verifier_id,
                f"verifier lease CAS failed before spawn: {exc}",
                role=AttemptRole.VERIFIER.value,
            )
            return False
        ...
        try:
            ...
            # 与 executor 同口径：verifier 也要签发一次性会话凭据。
            # 它是写验收证据的一方，没有凭据就等于任何人都能替它写证据。
            input_ = dataclasses.replace(
                input_, session_token=_issue_session_token(self._conn, verifier_id)
            )
        except CapacityRefusedError as exc:
            # 租约已占，编译失败必须走 _fail_attempt（它负责释放租约），
            # 否则这条 verifier 的租约会一直挂着直到下一轮 reclaim。
            self._fail_attempt(
                now,
                contract_id,
                verifier_id,
                f"verifier context capacity refused: {exc}",
                role=AttemptRole.VERIFIER.value,
            )
            return False
        try:
        ...
        self._conn.execute(
            ...
            (
                ...
                contract.revision,
                verifier_entry.id,
                verifier_entry.effective_model(),
                now.isoformat(),
        ...
        self._running[verifier_id] = {
            ...
            "role": AttemptRole.VERIFIER.value,
            "contract_revision": contract.revision,
            "model": verifier_entry.effective_model(),
            "session_ref": session_ref,
            "generation": lease.generation if lease else None,
        }
        self.spawned_count += 1
