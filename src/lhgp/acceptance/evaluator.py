...
def evaluate_check(
    ...
    *,
    ...
    # 阶段占位标记（stage_synth 合成，见
    # longtask.persistence.stage_synth.synthesize_stage_draft）：target 形如
    # ``stage:<id>``，不是 workspace 里的产物路径。协议层无从确定性判定
    # 阶段完成与否——必须如实记 undetermined、交给 §12.4 的 verifier 判定
    # 块填补「无法判定」的事实，而不是当文件路径去查、伪造 fail：协议
    # fail 优先于模型判定，会让所有自动合成的阶段合同在验收处一票否决。
    if spec.kind == CheckKind.ARTIFACT_PRESENT and spec.target.startswith("stage:"):
        return CheckResult(
            check_id,
            "undetermined",
            spec.target,
            "stage-completion marker is not a workspace artifact; defer to verifier verdict",
        )
