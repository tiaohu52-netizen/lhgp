...
class PortfolioSnapshot:
    ...
    def to_dict(self) -> dict[str, Any]:
        # ``ContractSummary`` is ``slots=True``: it has no ``__dict__``, so the
        # previous ``c.__dict__`` raised AttributeError for every non-empty
        # portfolio (``lhgp portfolio`` and the ``tool_portfolio`` MCP entry).
        # ``asdict`` is the slots-safe field reader.
        return {
