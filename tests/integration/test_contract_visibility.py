...
class TestWorkspaceExclusivity:
    ...
    def test_workspace_junction_like_alias_normalizes_to_target(self, tmp_path: Path) -> None:
        ...
        # ``os.symlink`` 返回成功不等于链接真的建出来了：某些沙箱/过滤驱动会
        # 静默 no-op（不抛异常、也不创建）。缺这道断言时，测试会拿一个并不
        # 存在的别名路径去比 realpath，报出一个与生产代码无关的假失败。
        # 需要真链接时用 junction（``cmd /c mklink /J``），realpath 能解析。
        if not link.is_symlink():
            pytest.skip("os.symlink was silently ignored on this platform")
...
class TestPerTickCapacityCap:
    """``max_dispatches_per_tick`` 必须真的限制单 tick 派工数。

    回归：``tick.py`` 只调用了 ``TickCapacityLedger.record_dispatch``，
    从不调 ``can_dispatch``——注释与 ``fairness.py`` 的 docstring 都声称
    "规则 2 已接线"，实际无论配成什么都无效。
    """
    ...
    def _registry() -> object:
        ...
        reg.register(
            RegistryEntry(
                ...
                capabilities=caps,
                limits={"max_concurrent_attempts": 8},
                cost_hint=CostHint.LOW,
                enabled=True,
            )
        )
    ...
    def _setup(self, tmp_path: Path) -> Path:
        """两份合同、各自独立的 workspace（避开排他串行化）。"""
        data_dir = tmp_path / "data"
        data_dir.mkdir()
        save_and_activate(data_dir, "lt-capA", make_draft(workspace=str(tmp_path / "wsA")))
        save_and_activate(data_dir, "lt-capB", make_draft(workspace=str(tmp_path / "wsB")))
        (tmp_path / "wsA").mkdir(exist_ok=True)
        (tmp_path / "wsB").mkdir(exist_ok=True)
        return data_dir

    def test_cap_of_one_limits_dispatch_to_a_single_contract(self, tmp_path: Path) -> None:
        from lhgp.promoter.fairness import FairnessConfig
        from longtask.cli.tick import run_daemon_tick
        ...
        try:
            res = run_daemon_tick(
                ...
                self._registry(),
                now=NOW,
                fairness_config=FairnessConfig(max_dispatches_per_tick=1),
            )
        finally:
            conn.close()
        assert len(res["attempts_started"]) == 1

    def test_default_config_places_no_cap(self, tmp_path: Path) -> None:
        """不配置（cap=0）时行为必须与既往一致——不因为接线而误伤。"""
        from longtask.cli.tick import run_daemon_tick

        data_dir = self._setup(tmp_path)
        conn = connect(StoreConfig(db_path=data_dir / "state.db"))
        try:
            res = run_daemon_tick(data_dir, conn, self._registry(), now=NOW)
        finally:
            conn.close()
        assert len(res["attempts_started"]) == 2

    def test_cap_of_one_is_per_tick_not_per_contract(self, tmp_path: Path) -> None:
        """帽是 per-tick 的：第二个 tick 可以派另一个合同（不会永久饿死）。"""
        from lhgp.promoter.fairness import FairnessConfig
        from longtask.cli.tick import run_daemon_tick

        data_dir = self._setup(tmp_path)
        cfg = FairnessConfig(max_dispatches_per_tick=1)
        conn = connect(StoreConfig(db_path=data_dir / "state.db"))
        try:
            first = run_daemon_tick(data_dir, conn, self._registry(), now=NOW, fairness_config=cfg)
            seen = {s["contract_id"] for s in first["attempts_started"]}
            # 第一个 tick 已派过其中一个；第二个 tick 的记账是新的，
            # 因此另一个合同有机会（前提是它仍紧迫且没被同 workspace 挡住）。
            ...
            conn.close()
        assert len(seen) == 1
        assert len(second["attempts_started"]) <= 1
