...
def tool_deadline_report(args: dict[str, Any], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    for (cid,) in rows:
        ...
        if view is None:
            continue
        # 与 daemon 同口径：必须传 total_seconds，否则 elapsed_ratio 恒 0.5，
        # 未逾期合同永远报 NORMAL（CLI 的 deadline-report 曾同此病）。
        decision = compute_deadline_level(
            view.draft.deadline_at,
            now=now,
            total_seconds=contract_window_seconds(view.draft.deadline_at, view.created_at),
        )
...
def tool_write_back(args: dict[str, Any], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    与 ``tool_attach_to_executor`` 一致，附带本次 attempt 的会话凭据：
    ``LHGP_SESSION_TOKEN`` 由守护进程 spawn 时注入到被拉起进程的环境里，
    它证明「我确实是这个 attempt 被拉起的那个进程」。少了它，任何拿着
    主令牌的客户端都能替该 attempt 写回——verifier 的验收证据尤其不能
    这样被伪造。
    """
    return _mcp_route(
        Method.ATTEMPT_WRITE_BACK,
        {**args, "session_token": _get_session_token()},
        ctx,
    )
...
def _validate_arguments(args: dict[str, Any], schema: dict[str, Any]) -> str | None:
    """按工具 inputSchema 校验 arguments。

    覆盖 MCP schema 实际用到的关键字：required、未知键、type，以及
    ``enum`` / ``minimum`` / ``maximum`` / ``minLength`` / ``maxLength``
    / ``minItems`` / ``maxItems`` / ``pattern``。

    后一组此前**完全没查**：schema 里写了 ``enum`` 却照样放行任意值。
    MCP host 只保证把参数原样传过来，不会替服务端校验；``inputSchema``
    里声明了约束却不强制，等于把约束当成注释。返回错误信息字符串，
    None 表示通过。
    """
    ...
    for key, value in args.items():
        ...
        if isinstance(allowed, list) and allowed and value not in allowed:
            ...
            return f"parameter {key!r} must be one of [{shown}], got {value!r}"

        if isinstance(value, (int, float)) and not isinstance(value, bool):
            low = prop.get("minimum")
            if isinstance(low, (int, float)) and value < low:
                return f"parameter {key!r} must be >= {low}, got {value!r}"
            high = prop.get("maximum")
            if isinstance(high, (int, float)) and value > high:
                return f"parameter {key!r} must be <= {high}, got {value!r}"

        if isinstance(value, str):
            min_len = prop.get("minLength")
            if isinstance(min_len, int) and len(value) < min_len:
                return f"parameter {key!r} must be at least {min_len} chars"
            max_len = prop.get("maxLength")
            if isinstance(max_len, int) and len(value) > max_len:
                return f"parameter {key!r} must be at most {max_len} chars"
            pattern = prop.get("pattern")
            if isinstance(pattern, str) and re.search(pattern, value) is None:
                return f"parameter {key!r} must match pattern {pattern!r}"

        if isinstance(value, list):
            min_items = prop.get("minItems")
            if isinstance(min_items, int) and len(value) < min_items:
                return f"parameter {key!r} must have at least {min_items} item(s)"
            max_items = prop.get("maxItems")
            if isinstance(max_items, int) and len(value) > max_items:
                return f"parameter {key!r} must have at most {max_items} item(s)"
    return None
