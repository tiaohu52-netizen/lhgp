...
**通道编码（MUST）**：上述两条 stdout 通道（完成事件行与 `lhgp-verdict`
判定块）MUST 为 UTF-8。运行时按 UTF-8 解码子进程 stdout，非法字节以
U+FFFD 替换而非中断 attempt（宽容解码：编码问题不该让一次真实执行作废）。
为了让 Python 执行者满足该约定，适配器 spawn 时注入
`PYTHONIOENCODING=utf-8`；非 Python 执行者由 task_prompt 与适配器文档
声明同一义务。缺了这条约定，中文 Windows（cp936）等非 UTF-8 宿主上的
非 ASCII 证据会被静默替换成 U+FFFD——损坏不可逆、不报错，排查时只看到
「跑通了但证据是乱码」。
