...
**E2 状态（2026-09-11 审计 B8）**：已交付的只有「紧迫档降序派工」与「per-tick
容量记账（soft cap 默认 0 = 不限）」；本表要求的「饱和场景无饥饿」**未交付**——
`fairness.py` 的饥饿规则（连续 5 tick 未派工 → 提到队首）从未被调用（tick 传入的
状态 dict 恒为空，`observe_tick` 只出现在测试里），而 DESIGN §8.3 承诺的是另一套
升级语义（额度不足 + u ≥ 1.0 → `blocked(need-user)`，默认 10 轮），两者的阈值与
动作都不一致。这正是本行「依赖：独立 SPEC 与测试」要解决的问题：先定 SPEC（统一
阈值与动作），再接线，然后才谈「无饥饿」。`v0.1.0a6` 的 CHANGELOG 曾宣称饥饿检测
已交付，已在 CHANGELOG 就地更正。
