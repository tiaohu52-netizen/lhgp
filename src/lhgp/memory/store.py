...
def record_memory(
    ...
    *,
    ...
    try:
        # ``transaction`` 而非裸 ``with conn:``：后者会**提交**任何已在进行的
        # 外层事务。``record_memory`` 会被 ``feedback.record_evaluation`` 在
        # 调用方自己的事务里调用，裸 ``with conn:`` 会把评价行提前落库，
        # 使其与随后的 USER_EVALUATION_SUBMITTED 事件不再原子。
        # ``transaction`` 是可重入的：已在事务中则复用外层，不提前提交。
...
def expire_due(
    ...
    *,
    ...
    transaction. Daemon callers that also write an audit event should use
    ``_expire_due_in_transaction`` directly inside an outer
    ``with transaction(conn):`` so the two writes are atomic.

    Uses the re-entrant ``transaction`` helper, so calling this from inside
    an outer transaction no longer commits that transaction early.
    """
    with transaction(conn):
