        ...
        while max_cycles is None or cycles < max_cycles:
            ...
            if interval_seconds > 0:
                ...
                if next_at is not None:
                    ...
                    #
                    # 过期点不能映射成 sleep(0)（审计 B3 同族，2026-09-14）：
                    # 等待人工验收的合同被 tick 跳过，其过期决策点会长期滞留，
                    # 0 秒休眠把主循环变成 1Hz 空转（并让 L1 每秒重注册计划
                    # 任务）。本轮 tick 已处理一切可动作合同；滞后的过期值回落到
                    # 心跳间隔复查。
                    sleep_seconds = interval_seconds if until <= 0 else min(interval_seconds, until)
...
def _enforce_deadlines(
    ...
    for contract in list_contracts(conn, limit=1000):
        ...
        total_seconds = contract_window_seconds(deadline, contract.created_at)
