...
class TestDaemonLoopSleepsUntilDecisionPoint:
    ...
    def test_stale_past_decision_on_candidate_contract_sleeps_interval(
        self, tmp_path: Path
    ) -> None:
        """等待人工验收（CANDIDATE）合同的过期决策点不得让主循环空转。

        tick 跳过 CANDIDATE 合同（决策点不再被刷新）；若它停在过去，旧代码
        算出 sleep(0) → 1Hz 空转 + 每秒重注册计划任务（审计 B3 同族，
        2026-09-14 实测 schtasks 风暴）。修复后回落到心跳间隔。
        """
        from longtask.contracts.schema import AcceptanceStatus

        data_dir = tmp_path / "data"
        data_dir.mkdir()
        setup_contract(data_dir, "lt-p4cand", NOW + timedelta(hours=4))
        conn = connect(StoreConfig(db_path=data_dir / "state.db"))
        try:
            update_contract_state(
                conn,
                contract_id="lt-p4cand",
            ...
            set_next_decision_at(
                ...
                when=NOW - timedelta(minutes=5),
                now=NOW,
                reason="stale while awaiting user confirmation",
            )
        finally:
            conn.close()
        result, sleeps = self._run(data_dir, NOW + timedelta(hours=4))
