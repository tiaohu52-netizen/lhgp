    ...
    def refresh(
        ...
        *,
        ...
        for contract in list_contracts(conn, limit=1000):
            ...
            if earliest <= now:
                # 已到期的决策点由清醒的 daemon 本轮自行处理：L1 计划任务的
                # 职责是把**休眠中**的 daemon 唤醒，注册一个“现在”的任务毫无
                # 意义。旧实现把它钳到 now——等待人工验收的合同被 tick 跳过
                # （CANDIDATE 不再刷新决策点），过期值长期滞留，每个循环都以
                # 新的 now 重注册一次计划任务：实测 1Hz 的 schtasks /Create /F
                # 风暴 + 事件刷屏（审计 B3 同族，2026-09-14）。过期即本轮
                # 处理，不注册。
                continue
            # schtasks 只有分钟精度：按分钟槽登记，同一决策点不因秒级抖动被
            # 反复重建（每次 /Create /F 都是系统级副作用）。
            targets[cid] = earliest.replace(second=0, microsecond=0)
