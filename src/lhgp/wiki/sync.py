...
def _yaml_scalar(value: str) -> str:
    """Quote a string for use as a YAML frontmatter scalar.

    ``title`` 这类值是用户自由文本，未加引号时遇到 ``:``、``#``、引号、
    换行就会**破坏 frontmatter 结构**——Obsidian 读不到（或读到错误的）
    元数据，而且不报错。契约标题里出现冒号是常态（"Phase 1: 收尾"）。
    """
    escaped = (
        value.replace("\\", "\\\\")  # 反斜杠必须先转义，否则后面几步会互相污染
        .replace('"', '\\"')
        .replace("\r", "\\r")
        .replace("\n", "\\n")
    )
...
def _yaml_list(values: tuple[str, ...] | list[str]) -> str:
    if not values:
        return "[]"
    # 旧实现只转义了双引号、**没有转义反斜杠**：``a\\`` 会变成 ``a\`` 并
    # 吃掉后面的引号。统一走 _yaml_scalar 以保证转义顺序正确。
    quoted = [_yaml_scalar(str(v)) for v in values]
    return "[" + ", ".join(quoted) + "]"
...
def _render_page(
    ...
    *,
    ...
    fm_lines = [
        "---",
        "type: contract-page",
        f"id: {_yaml_scalar(view.contract_id)}",
